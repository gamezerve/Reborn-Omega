...
//		DEBUG_ASSERTCRASH(m_buddyButtonPushed,     ("No pushed button image for communicator in scheme %s!", m_name.str()));
		...
		win->winSetSize((m_chatLR.x - m_chatUL.x)*resMultiplier.x + (Real)COMMAND_BAR_SIZE_OFFSET,(m_chatLR.y - m_chatUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	}
	...
	if(win)
	{
		...
		win->winSetSize((m_workerLR.x - m_workerUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_workerLR.y - m_workerUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	...
	if(win)
	{
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_optionsLR.x - m_optionsUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_optionsLR.y - m_optionsUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	}
	...
	if(win)
	{
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_beaconLR.x - m_beaconUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_beaconLR.y - m_beaconUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	}
	...
	if(win)
	{
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_powerBarLR.x - m_powerBarUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_powerBarLR.y - m_powerBarUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
		DEBUG_LOG(("Power Bar UL X:%d Y:%d LR X:%d Y:%d size X:%d Y:%d",m_powerBarUL.x, m_powerBarUL.y,m_powerBarLR.x, m_powerBarLR.y, (m_powerBarLR.x - m_powerBarUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_powerBarLR.y - m_powerBarUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET  ));
	}
	...
	if(win)
	{
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_generalLR.x - m_generalUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_generalLR.y - m_generalUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	}
...
//		GadgetButtonSetHiliteSelectedImage(win, m_minMaxButtonPushed);
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_minMaxLR.x - m_minMaxUL.x)*resMultiplier.x + (Real)COMMAND_BAR_SIZE_OFFSET,(m_minMaxLR.y - m_minMaxUL.y)*resMultiplier.y + (Real)COMMAND_BAR_SIZE_OFFSET);
	}
	...
	if(win)
	{
		...
		win->winSetPosition(x,y );
		win->winSetSize((m_uAttackLR.x - m_uAttackUL.x)*resMultiplier.x+ (Real)COMMAND_BAR_SIZE_OFFSET,(m_uAttackLR.y - m_uAttackUL.y)*resMultiplier.y+ (Real)COMMAND_BAR_SIZE_OFFSET);
	}
