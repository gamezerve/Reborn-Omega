...
#elif defined(USE_DEBUG_ON_LIVE_SERVER) || defined(REBORN_BUILD)
	// Reborn: Custom development builds must use the public GO service, not localhost.
	const static EEnvironment g_Environment = EEnvironment::PROD;
#pragma message ("Building for PROD environment (Custom Client)")
#else
	#if defined(_DEBUG)
	...
	~NGMP_OnlineServicesManager(); // Reborn: Moved to OnlineServices_Init.cpp 
